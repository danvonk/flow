...
struct MovingWallVelocityStencil {
  ...
  {
    field.v.u(i, j) = 2 * c_params.wall.vector_top[0] - field.v.u(i, j - 1);
    field.v.v(i, j - 1) = c_params.wall.vector_top[1];
  }

  __device__ void apply_bottom_wall(FlowFieldView &field, int i, int j)
  {
    field.v.u(i, j) = 2 * c_params.wall.vector_bottom[0] - field.v.u(i, j + 1);
    field.v.v(i, j) = c_params.wall.vector_bottom[1];
  }
};
...
struct MovingWallFGHStencil {
  ...
  __device__ void apply_left_wall(FlowFieldView &field, int i, int j)
  {
    field.fgh.u(i, j) = c_params.wall.vector_left[0];
  }

  __device__ void apply_right_wall(FlowFieldView &field, int i, int j)
  {
    field.fgh.u(i - 1, j) = c_params.wall.vector_right[0];
  }

  __device__ void apply_bottom_wall(FlowFieldView &field, int i, int j)
  {
    field.fgh.v(i, j) = c_params.wall.vector_bottom[1];
  }

  __device__ void apply_top_wall(FlowFieldView &field, int i, int j)
  {
    field.fgh.v(i, j - 1) = c_params.wall.vector_top[1];
  }
};
} // namespace stencils
